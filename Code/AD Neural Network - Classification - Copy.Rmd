...
* Output Layer - 2 units (represents the output classes, largechange or smallchange)
...
train3 <- train2 %>% 
  ...
  mutate(perc_change = (month3 - baseline)/baseline,
         largechange = case_when(perc_change >= .1 | perc_change <= -.1 ~ 1,
                              TRUE ~ 0),
         smallchange = case_when(largechange == 0 ~ 1,
                                 TRUE ~ 0)) 
...
sum_tab <- train3 %>% 
  ...
  summarise(n_pat = n(),
            ...
            n_MILDAD = sum(MILDAD),
            n_MCI = sum(MCI),
            n_largechange = sum(largechange),
            n_smallchange = sum(smallchange))
...
train %<>%
  mutate(largechange = as.factor(largechange))

train_index <- createDataPartition(train$largechange, p = 0.8, list = FALSE, times = 1)

train %<>%
  mutate(largechange = as.numeric(as.character(largechange)))
...
predict <- function(X, y, parameters) {
  ...
  for(i in 1:ncol(X)) {
    ...
    predvec <- A2[["A"]]
    if(which(predvec == max(predvec)) == 1) {pred =  "patient largechange"} else {pred = "patient smallchange"}
    if(which(truevec == max(truevec)) == 1) {true = "patient largechange"} else {true = "patient smallchange"}
