...
train3 <- train2 %>% 
  select(-VISITNUM) %>% 
  filter(time_since_first_visit == 0 | between(time_since_first_visit, 502, 592)) %>% 
  ...
  filter(!is.na(baseline) & !is.na(month18)) %>% 
  ungroup() %>% 
  mutate(SEX = case_when(SEX == "F" ~ 0,
         ...
         MCI = case_when(DIAGNOSIS == "MILD COGNITIVE IMPAIRMENT" ~ 1,
                         TRUE ~ 0)) %>% 
  select(AGE, SEX, RACE, ETHNIC, AD, MILDAD, MCI, baseline, month18) %>% 
  filter(AGE != 999) %>% 
  mutate(AGE = AGE/max(AGE),
         baseline = baseline/70,
         month18 = month18/70) %>% 
  rowwise() %>% 
  mutate(perc_change = (month18 - baseline)/baseline,
...
ggplot(train3, aes(x = month18*70)) +
  geom_histogram() +
  theme_pubr() +
  ggtitle("ADAS-COG 11 at Month 3") +
  xlab("ADAS-COG 11 Score")

ggsave("../Figures/month18_dv_dist.png", width = 16, height = 9, units = "cm")
...
initialize_parameters <- function(n_x, n_h, n_y) {
  ...
  W2 <- matrix(runif(xxx * xxx), xxx, xxx)*.01 n
